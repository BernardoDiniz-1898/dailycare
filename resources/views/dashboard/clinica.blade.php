...
<div style="max-width:1024px; margin:0 auto; display:flex; flex-direction:column; gap:28px;">
    ...
    <section aria-label="Metricas rapidas"
        ...
        @php
            $kpis = [
                ...
                [
                    ...
                    'cor' => '#EF4444',
                    'fundo' => '#FEF2F2',
                    'link' => route('dashboard', ['status' => 'solicitado']),
        ...
        @foreach ($kpis as $kpi)
            <a href="{{ $kpi['link'] }}"
               style="display:flex; flex-direction:column; justify-content:center; min-height:140px; padding:20px; background:#fff; border:1px solid #F3F4F6; border-radius:16px; box-shadow:0 1px 2px rgba(0,0,0,0.05); text-decoration:none; position:relative; transition:transform 0.2s ease, box-shadow 0.2s ease, border-color 0.2s ease;">
        ...
        @if ($agendaHoje->count() > 0 || $slotsHoje->count() > 0)
            <div style="display:flex; flex-direction:column; gap:12px;">
                @if ($slotsHoje->count() > 0)
                    @foreach ($slotsHoje as $slot)
                        @php $agendamento = $slot['agendamento']; @endphp
                        @if ($agendamento)
                            @php $paciente = $agendamento->paciente; @endphp
                            <article class="card" style="border-left:4px solid #009688; padding:16px; border-radius:16px;" aria-label="Agendamento de {{ $paciente->nome }} as {{ $slot['hora'] }}">
                        ...
                        @else
                            <article style="border-left:4px dashed #E5E7EB; background:#F9FAFB; border-radius:16px; padding:16px;" aria-label="Horario livre as {{ $slot['hora'] }}">
                                <div style="display:flex; align-items:center; gap:14px; flex-wrap:wrap;">
        ...
        @if ($ultimasAvaliacoes->count() > 0)
            <div style="display:flex; flex-direction:column; gap:12px;">
                @foreach ($ultimasAvaliacoes as $avaliacao)
                    <article style="background:#F9FAFB; border:1px solid #F3F4F6; border-radius:16px; padding:16px;" aria-label="Avaliacao de {{ $avaliacao->paciente->nome }}">
    ...
    {{-- =============================================
         TODOS OS AGENDAMENTOS (com filtro por status)
         ============================================= --}}
        ...
        <div style="display:flex; justify-content:space-between; align-items:flex-end; flex-wrap:wrap; gap:16px; margin-bottom:16px;">
            <div>
                <h2 style="font-size:1.25rem; font-weight:800; color:#111827; font-family:var(--font-heading); margin:0;">
                    <i class="bi bi-clipboard-data" aria-hidden="true"></i> Todos os agendamentos
                </h2>
                <p style="color:var(--color-text-secondary); margin:2px 0 0;">
                    {{ $clinica->nome_fantasia }}
                    @if ($clinica->preco_sessao)
                        &middot; R$ {{ number_format($clinica->preco_sessao, 2, ',', '.') }}/sessão
                    @endif
                </p>
            </div>
                        ...
                        <div style="display:flex; justify-content:space-between; align-items:flex-start; flex-wrap:wrap; gap:16px;">
                            <div style="flex:1; min-width:240px;">
                                ...
                                <p style="color:var(--color-text-secondary); font-size:0.9375rem; margin:0;">
                                    ...
                                    {{ \Carbon\Carbon::parse($agendamento->data)->translatedFormat('l, d/m/Y') }}
